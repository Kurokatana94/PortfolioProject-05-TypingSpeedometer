...
# Start button ------------------------------------------
def disable_button():
    start_button.config(command=lambda: None)  # Disable logic
    start_button.config(image=disable_btn_im)  # Keep original image
    # Optional: add a tooltip or visual cue that it's "disabled"

def enable_button():
    start_button.config(command=text_to_type_show)
...
def validate(text):
    if len(text) == 1 and text not in ACCEPTED_CHARS and text != ' ':
        return False
    return True
...
user_entry.bind('<Key>', lambda event: [check_correct_char(event), timer_start()])
# ----------------------------------------------------------------
# Timer----------------------------------------------------
